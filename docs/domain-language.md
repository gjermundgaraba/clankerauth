...
| Issuer               | The authorization server's exact `CLANKERAUTH_BASE_URL/api/auth` identifier.                                                                                                                                                                                                 |
| Client               | A registered OAuth participant that requests tokens. One client ID may have access to several resources. Public clients use PKCE without a secret; confidential clients also authenticate with a secret. Managed clients are first party; automatic clients are third party. |
| Resource             | A protected target identified by an exact audience URL in canonical form (`new URL(id).href === id`), with a dashboard-managed name and scopes. It is the target of client access and consent.                                                                               |
...
| Scope                | A named permission defined by a resource. Identical labels on different resources can have different meaning and independent consent. `offline_access` is a protocol scope, not a resource permission; there are no OpenID Connect scopes.                                   |
...
| Consent              | The owner's approval for an automatic client to receive particular scopes for one resource. Approving another resource does not replace this consent. Managed clients skip consent unless the request prompts for it.                                                        |
...
See [provider lifecycle](provider-integration.md#client-and-resource-lifecycle) for how access, scope and resource changes affect stored authorization, and [concurrency and transactions](provider-integration.md#concurrency-and-transactions) for in-flight requests.
