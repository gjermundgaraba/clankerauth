...
For a compatible MCP client, paste the MCP server URL into the client, sign in as the owner, and approve the requested Resource and scopes. Create the Resource in this dashboard first and configure the MCP server to advertise this issuer. Dynamic Client Registration (DCR) lets clients register automatically without manually copying a client ID or callback URL. CIMD is disabled: private-network clients can use DCR without requiring the issuer to fetch a public client metadata document.
...
The dashboard lists managed and dynamically registered clients, plus any retained legacy CIMD records. Legacy CIMD clients must register again through DCR before authorizing or refreshing; stored grants are not deleted by this change. **Revoke authorization** clears consent and renewal credentials while allowing a new consent flow. **Block client** additionally prevents authorization for that client ID until unblocked; existing CIMD records remain manageable, but new metadata discovery is disabled. Unblocking does not restore old grants. Existing JWTs at offline downstream verifiers may remain valid for up to five minutes. A block is specific to an identifier, not an attestation of the software or its operator.
...
1. Discover metadata from the pinned issuer. Register through the advertised `registration_endpoint` (DCR), or use a managed client ID. The issuer does not advertise CIMD support. Generate a random PKCE verifier and S256 challenge; retain verifier, state, and expected issuer in the client.
...
The service supports DCR and managed pre-registration. CIMD discovery is disabled, so the issuer does not fetch client metadata URLs or advertise `client_id_metadata_document_supported`. Existing stored clients and grants are retained. DCR is unauthenticated so compatible clients can onboard automatically. Registration is limited to 10 requests per minute per direct peer and 1,000 automatic client records. On new onboarding attempts, unblocked clients older than seven days with no stored consent, refresh grants, or pending authorization codes are removed; blocked identifiers are retained. Treat registration as untrusted metadata and apply registration abuse limits at the reverse proxy. The official MCP client SDK interoperability harness exercises real clanker-okf actions with isolated fake storage; see [reproduction and results](docs/mcp-interop.md). Named desktop MCP products still require validation in the target environment.
