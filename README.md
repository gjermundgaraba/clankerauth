...
The **issuer is `AUTH_BASE_URL/api/auth`**, not the origin alone. All integrations must pin that exact value. The server reconstructs requests from configured `AUTH_BASE_URL`, not `Host`, `Forwarded`, or `X-Forwarded-*`. Use one canonical hostname; proxies must not rewrite the public path. Only the canonical origin is trusted for account setup, owner login, consent and administrative writes. Browser cookies are HttpOnly, SameSite=Lax, and Secure on HTTPS. Resource and client administration requires a valid owner session; no additional session-age limit applies.
...
This service assumes a clean, single-owner database. First-run setup creates its only account and permanently closes account creation; native signup remains unmounted. Every authenticated user is therefore the owner, and there are no legacy accounts or sessions to accommodate. Administrative routes require a valid session, while Better Auth validates sessions, OAuth grants and tokens at their respective endpoints. The owner marker records completed setup rather than defining a separate runtime role. Generic session JWT emission is explicitly disabled with `disableSettingJwtHeader`; `/get-session` returns no `set-auth-jwt` header. Public OAuth subject identifiers are used; pairwise subjects and machine grants are not configured.

Resources, their scopes, and client access are managed in the dashboard and stored in SQLite. A fresh database has no resources. Restarts retain dashboard changes; there are no environment seeds or configuration overrides. Resource identifiers are exact HTTP or HTTPS audience URLs without fragments, queries or credentials. Custom scopes are nonempty printable ASCII tokens excluding spaces, double quotes and backslashes; standard protocol scope names remain reserved. A resource owns its scope names: two resources can both define `read` with separate meaning and consent. See [domain language](docs/domain-language.md) for the shared vocabulary.

Downstream apps may rely on this issuer's single-account identity model; they do not need a separate configured owner ID or `sub` equality check. They must still verify the access token's signature, exact issuer, token type, expiry and their own resource audience, and enforce the required endpoint scopes. Resource registration alone does not grant access: the verified token must carry the required scopes. Keep `sub` for identity and audit attribution. This contract depends on the single-account model; adding other users or grant types requires reviewing issuer authorization and downstream access policy. Do not authorize by email or assume `email_verified` is true; this service has no email verification transport.
...
Sign in and use **Register client**. Supply **Client name** and **Exact redirect URI**, and optionally select its **Allowed Resources**. You can register a client before creating Resources and configure its access later. Public desktop/native clients cannot keep secrets: select **Native / desktop client (loopback redirect)** and leave **Confidential client (can securely store a secret)** unchecked. Web backends can select confidential and securely store the one-time secret. Confidential clients use `client_secret_basic`; public clients use `none`. All clients require S256 PKCE and consent. Redirect validation comes from Better Auth (HTTPS for web, exact loopback/private-use schemes for native; loopback ports follow native-client rules).

A managed client can access multiple resources through its **Client access** settings; automatic clients need no manual assignment. Each authorization and token request targets exactly one resource, with independent consent. Missing, repeated or multiple `resource` parameters are rejected. Access and scope edits change policy while retaining stored authorization; use **Revoke authorization** to clear it explicitly. See [provider lifecycle](docs/provider-integration.md#client-and-resource-lifecycle) for relinking, deletion and credential behavior.
...
- `POST /admin/clients`: `{name, redirect, resources: string[], native, confidential}` (`resources` may be empty); returns client metadata and a one-time secret when confidential.
...
- `POST /admin/resources/delete`: `{identifier}`; deletes a resource and its client access links.
...
Requests run concurrently, and refresh rotation can race with revocation: a replacement refresh token may survive a successful revocation response. Run only one process/Service instance per database. See [concurrency and transactions](docs/provider-integration.md#concurrency-and-transactions) and the [provider patch and shutdown contract](docs/provider-integration.md#provider-patch-and-shutdown) for the guarantees and limits.
